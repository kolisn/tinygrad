...
class ConvAutoEncoder:
    ...
    def encode(self, x):
        ...
        Args:
          x: image tensor of shape (batch, 32, 32, 3)
        
        Returns:
          latent tensor of shape (batch, 4, 4, 128)
        """
    ...
    def decode(self, z):
        ...
        Args:
          z: latent tensor of shape (batch, 4, 4, 128)
        
        Returns:
          reconstructed image of shape (batch, 32, 32, 3) with values in [0,1]
        """
...
# Use only 1/10 of the dataset for faster training
X_train = X_train[:len(X_train)//10]
X_test = X_test[:len(X_test)//10]
print(f"✓ Using 1/10 of dataset: X_train shape: {X_train.shape}, X_test shape: {X_test.shape}")
...
if __name__ == "__main__":
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx in range(total_batches):
            ...
            # Print first 5 batches and every 10th
            should_print = (batch_idx < 5) or ((batch_idx + 1) % 10 == 0)
            if should_print:
    ...
    for row in range(3):
        for col in range(5):
            z_random = Tensor.randn(1, 4, 4, 128)
